//API8ASSQ JOB (ACCT#),'SYLVAIN',MSGCLASS=H,REGION=4M, 
//    CLASS=A,MSGLEVEL=(1,1),NOTIFY=&SYSUID,COND=(4,LT) 
//* 
//*  ETAPE DE COMPILATION 
//* 
//COMPIL   EXEC IGYWCL,PARM.COBOL=(NODYNAM,OBJECT,LIB,TEST,APOST) 
//COBOL.SYSIN  DD DSN=API8.SOURCE.COBOL(ASSEQSTR),DISP=SHR 
//COBOL.SYSLIB DD DSN=CEE.SCEESAMP,DISP=SHR 
//             DD DSN=API8.SOURCE.COPY,DISP=SHR 
//* 
//*  ETAPE DE LINKEDIT 
//* 
//LKED.SYSLMOD DD DSN=API8.COBOL.LOAD,DISP=(SHR,KEEP,KEEP) 
//LKED.SYSIN DD * 
 NAME ASSEQSTR(R) 
/* 